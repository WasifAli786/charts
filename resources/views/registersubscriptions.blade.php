    ...
    <header class="h-screen fixed">
        <div class="bg-eerie_black h-full py-4 border-r border-outer_space-500">
            ...
            <ul class="py-4 flex flex-col space-y-3 text-sm px-2 mt-4">
                <li><a href="/controlpanel/all">All Users</a></li>
            ...
            <div class="w-full h-16 my-2">
                ...
                <table class="mt-8 w-full">
                    <thead class="border-b border-outer_space-500 bg-onyx">
                        <tr>
                            <td class="py-4 ps-2">Subscriber</td>
                            <td class="py-4">Requested</td>
                            <td class="py-4">Status</td>
                            <td class="py-4">Authorize</td>
                        </tr>
                    </thead>
                    <tbody id="table" class="divide divide-y divide-slate-600 text-sm bg-eerie_black">
                        @foreach ($requests as $request)
                            <tr>
                                <td class="py-4 ps-2">
                                    {{ \App\Models\User::find($request->user_id)->first('email')->email }}
                                </td>
                                <td class="py-4">
                                    {{ \App\Models\User::find($request->trader_id)->first('email')->email }}
                                </td>
                                <td class="py-4 capitalize">
                                    {{ $request->status }}
                                </td>
                                <td class="py-4 flex space-x-2">
                                  <form action="/authorize/{{ $request->id }}" method="post">
                                    @csrf
                                    @method('PATCH')
                                    <button class="py-2 rounded-md px-2 bg-green-400">Authorize</button>
                                  </form>
                                  <form action="/decline/{{ $request->id }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <button class="py-2 rounded-md px-2 bg-red-600">Decline</button>
                                  </form>
                                </td>
                            </tr>
                        @endforeach
